...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  ...
  private:
    ...
    // this means which variant we are currently running
    uint32_t m_variant;
    ...
    // ANIK
    /// Broadcast ID for RevRequest
    uint32_t m_revRequestId;
    ...
    // ANIK
    /// Handle duplicated REV_RREQ
    IdCache m_revRreqIdCache;
  ...
  private:
    ...
    // ANIK
    void RecvRevRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src);
    /**
     * Receive RREP
     * \param p packet
     * \param my destination address
    ...
    // ANIK
    void SendRevRequest(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin);
    /** Send REV_RREQ 
    ...
    // ANIK-NS3-OFFLINE-1
    /// Map IP address + Alternative route.
    std::map<Ipv4Address, std::vector<RoutingTableEntry>> m_alternativeRoutes;
};
