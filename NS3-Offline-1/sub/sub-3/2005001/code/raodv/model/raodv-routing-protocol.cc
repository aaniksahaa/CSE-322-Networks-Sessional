...
RoutingProtocol::SendRevRequest(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j) {
        ...
        // setting the destination for this interface, as we are broadcasting
        revRreqHeader.SetDst(iface.GetLocal());
...
// because we are now broadcasting, intermediate nodes do not necessarily have a route
// to the origin
...
RoutingProtocol::RecvRevRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    // check two things, whether this is a duplicate broadcast REV_RREQ
    ...
    // Setting Neighbour Routes
    // This is much like as done in RecvRequest
    RoutingTableEntry toNeighbor;
    if (!m_routingTable.LookupRoute(sender, toNeighbor))
    {
    ...
    else
    {
        toNeighbor.SetLifeTime(m_activeRouteTimeout);
        toNeighbor.SetValidSeqNo(false);
        toNeighbor.SetSeqNo(revRreqHeader.GetDstSeqno());
        toNeighbor.SetFlag(VALID);
        toNeighbor.SetOutputDevice(m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver)));
        toNeighbor.SetInterface(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0));
        toNeighbor.SetHop(1);
    ...
    if (isMeOrigin) {
        ...
        if (!isDuplicate) {
            LOG_UNCOND_GREEN("Attention! First REV_RREQ received by origin. Now it will send packets in Queue.");
            ...
            // the paper just mentions that
            // these are stored as alternative routes
            // no specific details is found regarding how ther are later used
            m_alternativeRoutes[dst].push_back(toDst);
        }
        return;
    }
