...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (m_rreqIdCache.IsDuplicate(origin, id))
    {
        ...
        // actually simulates multicast instead of braodcast in original RAODV
        if(m_variant == 2){
    ...
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
        ...
        // however, for the one unicast route, we already had found it as shortest
        // so there is some probability that this shortest has not broken
        // in the meantime
        if(m_variant == 1){
            LOG_UNCOND_YELLOW("Attention! Destination reached. Sending RREP message.");
            NS_LOG_DEBUG("Send reply since I am the destination");
