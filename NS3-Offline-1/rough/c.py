...
def parse_filename(filename):
    pattern = r'experiment_protocolName-(\w+)_variant-(\d+)_nodeCount-(\d+)_packetsPerSecond-(\d+)_nodeSpeed-(\d+)_flowRunningTime-(\d+)_comment-(\w+)\.'
    ...
    if match:
        return {
            'protocolName': match.group(1) + (f"-v{variant}" if variant>0 else ""),
            'nodeCount': int(match.group(3)),
            'packetsPerSecond': int(match.group(4)),
            'nodeSpeed': int(match.group(5)),
            'flowRunningTime': int(match.group(6)),
